...
if (!hacsConfig) {
  ...
  check(hacsConfig.filename, `filename: "${hacsConfig.filename}"`, 'filename is required');
  check(hacsConfig.render_readme !== undefined, `render_readme: ${hacsConfig.render_readme}`, 'render_readme recommended', true);
}
...
if (hacsConfig && hacsConfig.filename) {
  ...
  check(bundleExists, `Bundle found: ${distPath}`, `Bundle not found: ${distPath} (run: npm run build)`);
  ...
  if (bundleExists) {
    ...
    check(size < 5 * 1024 * 1024, '', `Bundle too large (${sizeMB} MB) - HACS may reject`, false);
    
    if (size > 1024 * 1024) {
      console.log(`  ⚠️  Bundle > 1MB - consider optimization (current: ${sizeMB} MB)`);
      warnings++;
    }
...
check(readmeExists, 'README.md found', 'README.md not found (required for HACS)');

if (readmeExists) {
  const readme = fs.readFileSync(path.join(ROOT, 'README.md'), 'utf8');
  check(readme.length > 500, `Length: ${readme.length} chars`, 'README seems too short (<500 chars)', true);
  check(
...
console.log('');
console.log('📜 LICENSE');
check(fileExists('LICENSE'), 'LICENSE found', 'LICENSE not found (recommended)', true);
...
check(hasReleaseWorkflow, 'Release workflow found', 'No release.yml - consider adding automated releases', true);

// Check if dist is in .gitignore
if (fileExists('.gitignore')) {
  const gitignore = fs.readFileSync(path.join(ROOT, '.gitignore'), 'utf8');
  if (gitignore.includes('dist')) {
    check(hasReleaseWorkflow, '', 'dist in .gitignore but no release workflow - HACS needs dist', true);
  }
}
